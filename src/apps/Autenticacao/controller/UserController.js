  ...
  async store(req, res) {
    try {
      ...
      try {
        ...
        const token = jwt.sign({ id:newUser.id,name:newUser.name, email:newUser.email, profile_id:newUser.profile_id}, process.env.TOKEN_SECRET, {
          expiresIn: process.env.TOKEN_EXPIRATION,
        });
    
        return res.json({ token, user: { nome: newUser.name, email:newUser.email } });
